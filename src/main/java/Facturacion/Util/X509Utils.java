...
import Facturacion.X500NameGeneral;
import Facturacion.KeyStoreProviderFactory;
import Facturacion.AutoridadesCertificantes;
import Facturacion.AutoridadesCertificantes;
import Facturacion.KeyStoreProviderFactory;
import Facturacion.X500NameGeneral;
...
//  }
//  
  public static String seleccionarCertificado(KeyStore keyStore, String tokenSeleccionado) throws KeyStoreException, IOException, NoSuchAlgorithmException, CertificateExpiredException, CertificateNotYetValidException, CertificateException {
    ...
    while (nombres.hasMoreElements()) {
      ...
      if (tokenSeleccionado.equals("SD_BIOPASS") || (tokenSeleccionado.equals("SD_EPASS3000") && x500emisor.getCN().contains(AutoridadesCertificantes.SECURITY_DATA.getCn()))) {
        if (AutoridadesCertificantes.SECURITY_DATA.getO().equals(x500emisor.getO()) && AutoridadesCertificantes.SECURITY_DATA.getC().equals(x500emisor.getC()) && AutoridadesCertificantes.SECURITY_DATA.getO().equals(x500sujeto.getO()) && AutoridadesCertificantes.SECURITY_DATA.getC().equals(x500sujeto.getC()) && (
          certificado.getKeyUsage()[0] || certificado.getKeyUsage()[1])) {
          aliasSeleccion = aliasKey;
          break;
        } 
        continue;
      } 
      if (tokenSeleccionado.equals("BCE_ALADDIN") || (tokenSeleccionado.equals("BCE_IKEY2032") && x500emisor.getCN().contains(AutoridadesCertificantes.BANCO_CENTRAL.getCn()))) {
      ...
      if (tokenSeleccionado.equals("ANF1") && x500emisor.getCN().contains(AutoridadesCertificantes.ANF.getCn()) && 
        AutoridadesCertificantes.ANF.getO().equals(x500emisor.getO()) && AutoridadesCertificantes.ANF.getC().equals(x500emisor.getC()) && AutoridadesCertificantes.ANF.getC().toLowerCase().equals(x500sujeto.getC()) && (
        certificado.getKeyUsage()[0] || certificado.getKeyUsage()[1])) {
        aliasSeleccion = aliasKey;
        break;
      } 
  ...
  public static String firmaValidaArchivo(File archivo, String dirPathSalida, String rucEmisor, String tokenID, String password) {
      ...
      if (aliaskey != null) {
        ...
        if (rucEmisor.equals(rucCertificado) && clavePrivada != null) {
          ...
          if (!(new ValidacionBasica()).validarArchivo(new File(archivoFirmado)))
            respuesta = "Se ha producido un error al momento de crear la firma del comprobante electrya que el la firma digital no es v"; 
